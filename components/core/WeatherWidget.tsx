...
const WeatherWidget = () => {
  ...
  useEffect(() => {
    const loadWeather = async () => {
      try {
        // Default to a central location if project location isn't available
        // Ideally this comes from project context
        const lat = 27.7172; // Kathmandu
        const lng = 85.3240;
    ...
    <Card className="h-full border-border/40 bg-white/60 dark:bg-slate-900/40 backdrop-blur-xl shadow-md overflow-hidden group hover:shadow-lg transition-all duration-300">
      ...
      <CardHeader className="pb-2 relative z-10">
        <div className="flex justify-between items-center">
          ...
          <Badge variant={weather.workableConditions ? "outline" : "destructive"} className="text-[10px] font-bold">
            {weather.workableConditions ? "Workable" : "Restricted"}
          </Badge>
        </div>
      </CardHeader>
