  ...
  async initialize(): Promise<void> {
    ...
    try {
      if ((window as any).initSqlJs && !_useSqlJs) {
        _SQL = await (window as any).initSqlJs({ locateFile: (file: string) => '/sql.js/sql-wasm.wasm' });
        _db = new _SQL.Database();
        _useSqlJs = true;
        return;
      }
      if ((window as any).SQL && !_useSqlJs) {
      ...
      console.warn('sql.js init failed, falling back to localStorage shim', err);
    }
